...
class FormInsertTestCase(BaseTestCase):
    def setUp(self) -> None:
        ...
        self.ctrl_voc_map.update(
            setup.add_study_related_ctrl_voc(self.ctrl_voc_endpoint)
        )
        self.prop_map.update(
            setup.add_study_related_properties(
                self.property_endpoint, self.ctrl_voc_map
            )
        )

    def test_insert_generic_study_form(self):
        self.form_map.update(
            setup.add_generic_study_form(self.form_endpoint, self.prop_map)
        )

    def test_insert_rna_seq_study_form(self):
        self.form_map.update(setup.add_rna_seq_form(self.form_endpoint, self.prop_map))
